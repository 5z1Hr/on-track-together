...
function updatePip(stage) { if (!pipWindow || pipWindow.closed) return; const q=(s)=>pipWindow.document.querySelector(s); q('.pip-plant').textContent=species[data.species]?.emoji||'🌳'; q('.pip-plant').style.transform=`scale(${[.08,.3,.55,.78,1][stage]})`; q('.pip-time').textContent=fmt(remaining); }
async function openMiniGarden(){
  ...
  pipWindow=await documentPictureInPicture.requestWindow({width:210,height:230});
  pipWindow.document.head.innerHTML=`<title>Mini Garden</title><style>*{box-sizing:border-box}body{margin:0;background:#f3f1e8;color:#17231c;font-family:ui-monospace,SFMono-Regular,monospace;height:100vh}.mini{width:100%;height:100%;display:grid;grid-template-rows:1fr auto;text-align:center;padding:10px;background:radial-gradient(#c9cabf 1px,transparent 1px);background-size:15px 15px}.garden{min-height:0;background:linear-gradient(#dff2d5 0 74%,#d8d0b7 74%);border:1px solid #17231c;display:grid;place-items:end center;overflow:hidden}.pip-plant{font-size:78px;line-height:1;transform-origin:center bottom;transition:transform .8s cubic-bezier(.2,.8,.2,1);filter:drop-shadow(0 5px 0 rgba(23,35,28,.12))}.pip-time{font-size:38px;font-weight:700;letter-spacing:-.08em;line-height:1;padding:12px 0 3px}</style>`;
  pipWindow.document.body.innerHTML='<div class="mini"><div class="garden"><div class="pip-plant"></div></div><div class="pip-time"></div></div>';
